...
package org.sample.mqttio.model;

import org.apache.commons.logging.Log;
import org.apache.commons.logging.LogFactory;
import org.eclipse.paho.client.mqttv3.MqttException;
import org.sample.mqttio.AndesMQTTClient;
import org.sample.mqttio.MQTTSampleConstants;
...
public class Vehicle {
	...
	public Vehicle(final String vehicleId, VehicleModel vehicleModel) throws MqttException {
		...
		mqttClient = new AndesMQTTClient(vehicleId, true, MQTTSampleConstants.DEFAULT_USER_NAME, MQTTSampleConstants.DEFAULT_PASSWORD);
		...
		statusUpdateSchedule = scheduledExecutorService.scheduleAtFixedRate(new Runnable() {
			public void run() {
				try {
					...
					mqttClient.sendMessage(generateTopicHierarchy(ENGINE_TEMPERATURE), org.sample.mqttio.AndesMQTTClient.TEMPERATURE_PREFIX + String.valueOf(getEngineTemperature()), qos);

					// Send speed reading
					mqttClient.sendMessage(generateTopicHierarchy(SPEED), org.sample.mqttio.AndesMQTTClient.SPEED_PREFIX + String.valueOf(getSpeed()), qos);

					// Send acceleration reading
					mqttClient.sendMessage(generateTopicHierarchy(ACCELERATION), org.sample.mqttio.AndesMQTTClient.ACCELERATION_PREFIX + String.valueOf(getAcceleration()), qos);
		...
		log.info("A " + vehicleModel.getVehicleType().getTypeName() + " of model " + vehicleModel.getModelName() + " " + "created. Id : " + vehicleId);
	}
	 ...
	 *
	 * @param leafTopic
	 *            The leaf of the topic hierarchy
	 * @return The topic hierarchy that is feed-able to the broker
	 */
	public String generateTopicHierarchy(String leafTopic) {
		return vehicleModel.getVehicleType().getTypeName() + "/" + vehicleModel.getModelName() + "/" + vehicleId + "/" + leafTopic;
	}
